...
class ReelsController extends BaseController
{
    ...
    public function listing(Request $request)
    {
        ...
        // Filtro para apenas vídeos (sintaxe PostgreSQL)
        ->whereRaw("media_link ~* '\\.(mp4|webm|ogg|mov|avi|wmv|flv|mkv|m4v)$'")
        ->groupBy([ 
            'posts.id',
            'posts.description', 
            'posts.media_link',
     ...
     * @return  Json
     */
    public function verticalReels(Request $request)
    {
        $userId = $request->getAttribute('user_id', false);
        $page = $request->getAttribute('page', 1);
        $perPage = $request->getAttribute('perPage', 5);
        ...
        ->whereNull('users.deleted_at')
        // Filtro para apenas vídeos (sintaxe PostgreSQL)
        ->whereRaw("media_link ~* '/_vertical[^/]*\\.(mp4|webm|ogg|mov|avi|wmv|flv|mkv|m4v)(\\?|$)'")
        ->groupBy([ 
            'posts.id',
            'posts.description', 
            'posts.media_link',
